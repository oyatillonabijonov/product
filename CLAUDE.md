...
bun run test           # vitest run (56 pure-logic tests: installment, auth, i18n, validate, catalog, variants, cart)
...
- Storefront pages: home, `category/:slug`, `product/:id`, `search`, `katalog` (all products + filter panel), `brand/:slug`, `chegirmalar` (deals), `savat` (cart, noindex).

### Core libraries (`app/lib/` server-shared, `src/lib/` client-shared — all pure & unit-tested)
- `app/lib/loaders.ts` — D1 reads + sample fallback: `loadStore`/`loadConfig`/`loadProductsBy`/`loadProductDetail`/`loadCategories`/`loadBrands`/`queryProducts` (dynamic WHERE + COUNT + brand/price facets; effective price = `COALESCE(min_variant_price, cash_price_uzs)`).
...
- `app/lib/i18n.ts` — locale ↔ `LangKey`, `resolveLocale`, `localizedPath`, `stripLocale`. `app/lib/seo.ts` — `pageTitle`/`organizationJsonLd`/`hreflangLinks`/`catalogMeta`. `app/lib/site.config.ts` — **the rebrand seam**: brand name, contacts, socials, map, SEO defaults.
...
Tabs: Products (ProductForm with brand select, slug, gallery, specs, **VariantEditor** — options + chips + "Kombinatsiyalarni generatsiya" + per-variant price/image/stock), Categories, Brands, Settings (calculator config). `src/admin/api.ts` is its fetch client. Admin UI convention: `FC<{...}>` components.

### Data model (D1)
Migrations in `migrations/` (numbered; **never edit an applied migration — add a new one**): `0001` products+settings, `0002` seed, `0003` storefront (categories, images, specs, category_id/old_price/description), `0004` product model (brands, options, variants, brand_id/slug). `shared/types.ts` is the API contract; `src/data/products.ts` holds frontend types + sample fallback data (keep its `brands`/`categories` in sync with migration seeds). Prices are **placeholder (NAMUNA)** — replaced via admin.
...
Filtered/paginated catalog pages get `noindex,follow` + canonical to the clean path (`catalogMeta`); `/search` and `/savat` are always noindex; sitemap enumerates home/katalog/chegirmalar/categories/brands/products across all locales.
