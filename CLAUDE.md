...
bun run test           # vitest run (135 pure-logic tests: installment, auth, i18n, validate, catalog, variants, cart, markdown, safe-href, seo + src/admin/lib helpers)
...
`src/locales.ts` — `translations` keyed by language name; **every key must exist in both languages** or the `Translation` type fails compilation (lint is the parity check). The site ships **two locales: `uz` (default) + `ru`** — English and Uzbek-Cyrillic were removed. Locale is **URL-prefixed** (`/` = uz default, `/ru`), resolved server-side; each page emits hreflang alternates. (The `pages` DB table still has legacy `_en`/`_cyrl` columns; they're never read.) Meta titles are localized in loaders via `translations[localeToLang(locale)]` (see `catalog.tsx`). Admin UI is Uzbek-only.
