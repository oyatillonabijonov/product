...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  await env.DB.prepare(
    'UPDATE site_config SET name=?, phone=?, phone_display=?, telegram=?, instagram=?, whatsapp=?, map_ll=?, seo_title_suffix=?, seo_description=?, og_image=?, payment_mode=?, telegram_bot_token=?, telegram_order_chat_id=?, google_client_id=?, google_client_secret=?, telegram_login_bot=?, yandex_metrica_id=?, billz_secret_token=?, billz_shop_id=? WHERE id=1',
  ).bind(input.name, input.phone, input.phoneDisplay, input.telegram, input.instagram, input.whatsapp,
    input.mapLl, input.seoTitleSuffix, input.seoDescription, input.ogImage, input.paymentMode,
    input.telegramBotToken, input.telegramOrderChatId,
    input.googleClientId, input.googleClientSecret, input.telegramLoginBot, input.yandexMetricaId,
    input.billzSecretToken, input.billzShopId).run();
  return json(input);
}
