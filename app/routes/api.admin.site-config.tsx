...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  await env.DB.prepare(
    'UPDATE site_config SET name=?, phone=?, phone_display=?, telegram=?, instagram=?, whatsapp=?, map_ll=?, map_label=?, seo_title_suffix=?, seo_description=?, og_image=?, payment_mode=?, telegram_bot_token=?, telegram_order_chat_id=?, google_client_id=?, google_client_secret=?, telegram_login_bot=?, yandex_metrica_id=?, billz_secret_token=?, billz_shop_id=? WHERE id=1',
