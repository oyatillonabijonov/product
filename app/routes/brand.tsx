...
export async function loader({ params, request, context }: Route.LoaderArgs) {
  ...
  return { result, config, brand, brands, filters, requestUrl: request.url, locale };
}
...
export function meta({ data, matches }: Route.MetaArgs) {
  ...
  const desc = t.metaCatalogDesc.replace('{title}', data.brand.name).replace('{store}', storeConfigFrom(matches)?.name ?? siteConfig.name);
