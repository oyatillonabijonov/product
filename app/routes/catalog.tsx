...
export async function loader({ params, request, context }: Route.LoaderArgs) {
  ...
  const metaTitle = translations[lang].catalogAll;
  return { result, config, brands, filters, requestUrl: request.url, metaTitle, locale };
}
...
export function meta({ data, matches }: Route.MetaArgs) {
  ...
  const desc = t.metaCatalogDesc.replace('{title}', data.metaTitle).replace('{store}', storeConfigFrom(matches)?.name ?? siteConfig.name);
