      ...
      <div className="flex overflow-x-auto gap-3 no-scrollbar pb-1">
        {brands.map((b) => (
          <LocaleLink
            key={b.id}
            to={`/brand/${b.slug}`}
            className="shrink-0 h-16 min-w-[120px] px-6 bg-white border border-line-3 rounded-2xl flex items-center justify-center shadow-apple hover:border-accent transition-colors"
          >
            {b.logoUrl
