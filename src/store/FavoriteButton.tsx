...
const FavoriteButton: FC<{ item: FavoriteItem; className?: string; addLabel: string; removeLabel: string }> = ({
  ...
  return (
    <button
      ...
      className={` press inline-flex items-center justify-center rounded-full bg-surface/90 border border-line-2 backdrop-blur-sm hover:bg-surface before:absolute before:-inset-1 before:content-[''] ${className}`}
    >
