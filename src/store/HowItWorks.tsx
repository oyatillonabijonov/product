...
export default function HowItWorks({ t }: { t: Translation }) {
  ...
  return (
    <section className="rounded-[28px] bg-[#F5F5F7] border border-[#ECECEF] px-6 md:px-10 py-9 md:py-11">
        ...
        {steps.map(({ icon: Icon, title, desc }, i) => (
          <div key={i} className="relative">
            <div className="flex items-center gap-3 mb-3">
              <span className="w-11 h-11 rounded-2xl bg-white text-[#0071E3] flex items-center justify-center shadow-[--shadow-apple]">
                <Icon className="w-5 h-5" strokeWidth={1.9} />
              </span>
              <span className="text-[13px] font-bold text-[#C7C7CC]">0{i + 1}</span>
            </div>
