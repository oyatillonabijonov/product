...
const inputCls = 'rounded-sm w-full border border-line-2 px-3.5 py-2.5 text-para text-primary focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/15 transition';
...
const PasswordForm: FC<{ t: Translation; hasEmail: boolean; hasPassword: boolean }> = ({ t, hasEmail, hasPassword }) => {
  ...
  if (!hasEmail) return <p className="text-muted text-label max-w-md">{t.pwNoEmail}</p>;
  ...
  return (
    <form onSubmit={submit} className="max-w-md flex flex-col gap-4">
      ...
      <div>
        ...
        <p className="text-label text-muted-2 mt-1.5">{t.loginPasswordHint}</p>
      </div>

      {err && <p className="text-sale text-label">{err}</p>}

      <div className="flex items-center gap-3 mt-1">
        <button type="submit" disabled={busy} className="press inline-flex h-11 items-center justify-center px-6 bg-accent text-bg font-normal rounded-full hover:bg-accent-hover disabled:opacity-50">
          {busy ? t.loginSubmitting : hasPassword ? t.pwChange : t.pwSet}
        </button>
        {saved && <span className="text-trust text-label font-medium">{t.pwSaved}</span>}
      </div>
    </form>
  );
};
